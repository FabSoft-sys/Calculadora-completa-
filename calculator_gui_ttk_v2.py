...
class CalculatorGUI:
    def __init__(self, master):
        ...
        buttons = [
            ...
            ('0', 4, 0), ('.', 4, 1), ('log', 4, 2), ('sin', 4, 3), ('cos', 4, 4), (')', 4, 5),
            ('tan', 5, 0), ('mean', 5, 1), ('median', 5, 2), ('mode', 5, 3), ('variance', 5, 4)
        ]
        ...
        try:
            ...
            while i < len(tokens):
                ...
                if token == 'raiz':
                    ...
                    # Para funções estatísticas, espera-se uma lista de números separados por vírgula após o token
                    if i + 1 < len(tokens):
                        list_str = tokens[i+1]
                        # Remover parênteses se existirem
                        list_str = list_str.strip('()')
                        # Converter string para lista de floats
                        num_list = [float(x) for x in list_str.split(',')]
                        if token == 'mean':
